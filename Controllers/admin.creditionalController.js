...
const adminCredentialController = {
  // GET /admin/credentials - Get all credentials (admin only)
 getAllCredentials: async (req, res, next) => {
  try {
    ...
    logger.error('adminGetAllCredentials', { message: error.message, stack: error.stack });
    next(error);
  }
},

  // GET /admin/credentials/:id - Get any credential (admin only)
...
// POST /admin/credentials - Create credential as any user (admin only)
 createCredential: async (req, res, next) => {
  try {
    const userId = req.payload.id; // Admin user ID
    const { 
      serviceName,
      type,
      subInstanceName,
      ...
      url,
      notes
      // Remove createdBy from destructuring
    } = req.body;
...
,
  ...
  // PUT /admin/credentials/:id - Update any credential (admin only)
  updateCredential: async (req, res, next) => {
    try {
      ...
      // Admin can update any credential without ownership check
      ...
      if (serviceName || type) {
        rootInstance = await RootInstance.findOne({
          ...
          createdBy: credential.createdBy // Keep original creator
        });
      ...
      logger.info('Admin updated credential', displayCredential);
  ...
  // DELETE /admin/credentials/:id - Delete any credential (admin only)
  ...
  // GET /admin/credentials/:id/decrypt - Get any decrypted credential (admin only)
  ...
  // GET /admin/credentials/:id/audit-logs - Get any credential's audit logs (admin only)
